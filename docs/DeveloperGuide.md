...
* stores the address book data i.e., all `Person` and `Appointment` objects (which are contained in a `UniquePersonList` and `AppointmentList` objects respectively).
