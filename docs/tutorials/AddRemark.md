...
/**
 * Changes the remark of an existing courseMate in the address book.
 */
...
```java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the courseMate identified "
            + "by the index number used in the last courseMate listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the courseMate in the filtered courseMate list to edit the remark
     * @param remark of the courseMate to be updated to
     */
...
Create a new `Remark` in `seedu.address.model.courseMate`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each courseMate.

Simply add the following to [`seedu.address.ui.CourseMateCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
```java
public PersonCard(Person courseMate, int displayedIndex) {
    //...
    remark.setText(courseMate.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person courseMateToEdit = lastShownList.get(index.getZeroBased());
        Person editedCourseMate = new Person(
                courseMateToEdit.getName(), courseMateToEdit.getPhone(), courseMateToEdit.getEmail(),
                courseMateToEdit.getAddress(), remark, courseMateToEdit.getTags());

        model.setPerson(courseMateToEdit, editedCourseMate);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedCourseMate));
    }

    /**
     * Generates a command execution success message based on whether
     * the remark is added to or removed from
     * {@code courseMateToEdit}.
     */
    private String generateSuccessMessage(Person courseMateToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, courseMateToEdit);
    }
```
