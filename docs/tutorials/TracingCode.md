...
According to the sequence diagram you saw earlier (and repeated above for reference), the `UI` component yields control to the `Logic` component through a method named `execute`. Searching through the code base for an `execute()` method that belongs to the `Logic` component yields a promising candidate in `scm.address.logic.Logic`.
...
A quick look at the `scm.address.logic.Logic` (an extract given below) confirms that this indeed might be what we’re looking for.
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save the contact manager in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Person`

    6.  Add a new entity to the contact manager
