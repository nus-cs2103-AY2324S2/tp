 ...
 */
public class ApplicationCommandParser implements Parser<ApplicationCommand> {
     ...
     */
    public ApplicationCommand parse(String args) throws ParseException {
        ...
        if (!arePrefixesPresent(argMultimap, PREFIX_EMAIL, PREFIX_TITLE)) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT,
                    ApplicationCommand.MESSAGE_USAGE));
        }
        ...
        if (argMultimap.getValue(PREFIX_STATUS).isPresent()) {
            ...
            return new ApplicationCommand(email, title, status);
        }

        return new ApplicationCommand(email, title);
    }
