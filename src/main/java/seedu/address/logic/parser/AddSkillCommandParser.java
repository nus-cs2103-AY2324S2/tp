...
import seedu.address.logic.commands.AddSkillCommand.SkillDescriptor;
...
public class AddSkillCommandParser implements Parser<AddSkillCommand> {
    ...
    public EditCommand parse(String args) throws ParseException {
        ...
        parseSkillsForEdit(argMultimap.getAllValues(PREFIX_SKILL)).ifPresent(skillDescriptor::setSkills);

        if (!skillDescriptor.isAnyFieldEdited()) {
            throw new ParseException(AddSkillCommand.MESSAGE_NOT_EDITED);
        }

        return new AddSkillCommand(index, skillDescriptor);
    }
