...
public class EditMeetingCommandParser implements Parser<EditMeetingCommand> {
    ...
    public EditMeetingCommand parse(String args) throws ParseException {
        ...
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_CLIENT_INDEX, PREFIX_MEETING_INDEX, PREFIX_NAME, PREFIX_DATETIME, PREFIX_CLIENT);

        Index meetingIndex;
        Index clientIndex;

        try {
            meetingIndex = ParserUtil.parseMeetingIndex(argMultimap.getPreamble());
        } catch (ParseException pe) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, EditMeetingCommand.MESSAGE_USAGE), pe);
        }

        try {
            clientIndex = ParserUtil.parseClientIndex(argMultimap.getPreamble());
        } catch (ParseException pe) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, EditMeetingCommand.MESSAGE_USAGE), pe);
        }
        ...
        return new EditMeetingCommand(meetingIndex, editMeetingDescriptor, clientIndex);
    }
